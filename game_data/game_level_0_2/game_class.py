...
class Game:
    ...
    def complete_move_phase(self):
        ...
        self.logs.write('\nBEGINNING OF TURN ' + str(turn_num) + ' MOVEMENT PHASE\n\n')
        for player_num in self.state['players']:
            ...
            for scout_num in player_scouts:
                ...
                self.logs.write('\tPlayer ' + str(player_num) + ' Scout ' + str(scout_num) + ': ' + str(scout) + ' -> ' + str(new_coords) + '\n')
        self.logs.write('\nEND OF TURN ' + str(turn_num) + ' MOVEMENT PHASE\n' + '-'*40)
    
    def complete_combat_phase(self):
        if self.state['winner'] != None:
            return None
        turn_num = self.state['turn']
        self.logs.write('\nBEGINNING OF TURN ' + str(turn_num) + ' COMBAT PHASE\n')
        ...
        for coord in battle_coords:
            ...
            self.logs.write('\n')
        self.logs.write('\nEND OF TURN ' + str(turn_num) + ' COMBAT PHASE\n' + '-'*40)
        self.state['turn'] += 1
